...
if(isset($_GET["t"]) && $_SERVER["REQUEST_METHOD"] === "POST") {
	...
	switch($type) {
		case "profile":
			...
			$sql = sprintf($arr_sql["update_profile"], $connect->real_escape_string($name), $_SESSION["user_id"], $id);
		break;
		case "label":
		...
		case "hour":
			...
			$sql = sprintf($arr_sql["update_hour"], $connect->real_escape_string($hour), $_SESSION["user_id"], $id);
		break;
		case "date":
			$date = htmlentities(trim(request("date")));
			$sql = sprintf($arr_sql["update_date"], $connect->real_escape_string($date), $_SESSION["user_id"], $id);
		break;

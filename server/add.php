...
if(isset($_GET["t"]) && $_SERVER["REQUEST_METHOD"] === "POST") {
	...
	switch($type) {
		case "profile":
			...
			$sql = sprintf($arr_sql["add_profile"], $connect->real_escape_string($name));
		break;
		case "label":
		...
		case "hour":
			...
			$sql = sprintf($arr_sql["add_hour"], $connect->real_escape_string($hour));
		break;
		case "date":
			$date = htmlentities(trim(request("date")));
			$sql = sprintf($arr_sql["add_date"], $connect->real_escape_string($date));
		break;
